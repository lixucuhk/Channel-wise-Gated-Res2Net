...
def prediction(val_loader, model, device, output_file, utt2systemID_file):
    ...
    with torch.no_grad():
        for i, (utt_list, input, target) in enumerate(val_loader):
            ...
            # compute output
            output = model(input)
            if model.loss == 'softmax':
                score = output[:,0] # use log-probability of the bonafide class for scoring
            elif model.loss == 'am-softmax':
                # index = torch.zeros_like(output[0])
                # index.scatter_(1, target.data.view(-1, 1), 1)
                # index = index.bool()
                # rst = output[0]*1.0
                # rst[index] -= output[0][index] * 1.0
                # rst[index] += output[1][index] * 1.0
                # score = F.log_softmax(rst, dim=-1)[:, 0]
                score = output[0][:,0]/model.cls_layer.s
            elif model.loss == 'oc-softmax':
                score = model.cls_layer.w_posi - output[0][:,0]/model.cls_layer.alpha
                # print(model.cls_layer.w_posi.size())
                # print(output[0].size())
                # print(model.cls_layer.alpha.size())
                # print(score)
                # exit(0)
            else:
                raise NameError
