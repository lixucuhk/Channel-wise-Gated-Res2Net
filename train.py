...
from local import datafiles, optimizer
from local import trainer as trainer
from local import validate as validate
...
def main(run_id, pretrained, data_files, model_params, training_params, device):
    ...
    # Data loading code
    train_data = SpoofDatsetSystemID(data_files['train_scp'], data_files['train_utt2index'], binary_class, data_files['isstft'])
    val_data   = SpoofDatsetSystemID(data_files['dev_scp'], data_files['dev_utt2index'], binary_class, data_files['isstft'])
