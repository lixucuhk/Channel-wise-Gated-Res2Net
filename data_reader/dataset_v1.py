...
class SpoofDatsetSystemID(data.Dataset):
    ...
    '''
    def __init__(self, scp_file, utt2index_file, binary_class, isstft=False):
    ...
    def __getitem__(self, counter):
        ...
        if self.isstft:
            X = read_mat(self.ark_dic[index])
            # print(X.shape)
            # print(X[0:20,5])
            feat_dim, time_dim = X.shape
            X = X.reshape(2, int(feat_dim/2), time_dim)
            # print(X.shape)
            # print(X[0,0:20,5])
            # exit(0)
        else:
